...
The metadata compiler walks request-body schemas once. A single cached analysis applies `$ref` and `allOf` as conjunction and records only serialization facts: declared types, `items`/`properties` hints, merged properties, conjoined item schemas, and binary markers. Property kinds, default part media and multipart `contentEncoding` restrictions are all derived from those facts, so equivalent schema spellings (inline, referenced, split across `allOf` in any order) compile to the same metadata. Explicit `type` values decide the shape before `items`/`properties` hints; explicit types that disagree on object, array or scalar shape are rejected rather than resolved by position.
