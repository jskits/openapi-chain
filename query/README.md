...
Include every response-affecting input in the operation input or key prefix, including a non-secret server/account/permission scope. Do not put credentials in keys. Keys and inputs must contain only finite JSON values: plain objects, arrays, strings, numbers, booleans and null. Omit optional fields instead of assigning undefined. Dates, BigInts, signals, functions, cyclic objects and sparse arrays are rejected before a request can start. Inputs and prefixes are copied and recursively frozen, so mutations after options creation cannot change a request behind its cache key. Fetchers must treat inputs as read-only.
