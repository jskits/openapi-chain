...
try {
  ...
  run(['exec', 'openapi-typescript', 'examples/example.yaml', '-o', output]);
  run(['exec', 'oxfmt', '--config', '.oxfmtrc.json', '--write', output]);
  assert.equal(
    readFileSync(output, 'utf8'),
    readFileSync(join(root, 'examples/schema.d.ts'), 'utf8'),
    'Generated fixture is stale. Run pnpm generate:example, then pnpm format.',
  );
